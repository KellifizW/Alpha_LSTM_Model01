...
from tensorflow.keras.optimizers import Adam  # 新增Adam優化器
...
def build_model(input_shape, model_type="original"):
    if model_type == "lstm_simple":  # 新增單層LSTM選項
        model = Sequential()
        model.add(LSTM(150, activation='relu', input_shape=input_shape))
        model.add(Dropout(0.01))  # 保留Dropout防過擬合
        model.add(Dense(1))
        model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=['mae'])
    else:  # 原有CNN-BiLSTM-Attention模型
...
def backtest(data, predictions, test_dates, period_start, period_end, initial_capital=100000):
    ...
    # 新增預測MACD（優先級1）
    data['EMA12_pred'] = pd.Series(predictions.flatten()).ewm(span=12, adjust=False).mean()
    data['EMA26_pred'] = pd.Series(predictions.flatten()).ewm(span=26, adjust=False).mean()
    ...
    for i in range(test_start_idx, test_end_idx + 1):
        ...
        macd_pred = data['MACD_pred'].iloc[i]  # 使用預測MACD
        ...
        # 止損規則（優先級2）
        if position == 1 and pred_price < close_price * 0.98:  # 預測下跌2%
...
def main():
    ...
    model_type = st.selectbox("選擇模型類型", ["original (CNN-BiLSTM-Attention)", "lstm_simple (單層LSTM 150神經元)"], index=0)  # 新增模型選擇
    ...
    if st.button("運行分析"):
        ...
        st.subheader("MACD 分析（回測期間，基於預測價格）")  # 更新標題
